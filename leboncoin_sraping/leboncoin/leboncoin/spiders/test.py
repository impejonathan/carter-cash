...
#         # Extraire la largeur, la hauteur et le diamètre de l'URL
#         url_parts = response.url.split('/')
#         largeur = int(url_parts[-3])
#         hauteur = int(url_parts[-2].split('-')[0])
#         diametre = int(url_parts[-1][1:])

#         item = {}
#         item["prix"] = response.xpath('//div[@class="price-container "]/div/div[@class="main-price"]/span[1]/text()').get()
#         item["info_generale"] = response.xpath('//h1/div/text()').get()
...
#             item["Bruit"] = "inconnue"
#      # //////       
#     # Vérifier si la largeur est dans le dictionnaire des largeurs
#         if largeur in largeur.keys():
#             item["Largeur"] = str(largeur)
#         else:
#             item["Largeur"] = None

#         # Vérifier si la hauteur est dans le dictionnaire des hauteurs
#         if hauteur in hauteur.keys():
#             item["Hauteur"] = str(hauteur)
#         else:
#             item["Hauteur"] = None

#         # Vérifier si le diamètre est dans le dictionnaire des diamètres
#         if diametre in diametre.keys():
#             item["Diametre"] = str(diametre)
#         else:
#             item["Diametre"] = None
#      # //////    
